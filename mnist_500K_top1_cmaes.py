...
if __name__ == "__main__":
    ...
    print(out)
    csv_path = f"out/{problem_name}_block_bs{block_size}_gfo_1000K-fe_top1_hist.csv"
    plt_path = f"out/{problem_name}_block_bs{block_size}_gfo_1000K-fe_top1_plt.pdf"
    model_path = f"out/{problem_name}_block_bs{block_size}_gfo_1000K-fe_top1_model"
    ...
    optimizer = CMA_ES(popsize=100, num_dims=bD, sigma_init=0.1, elite_ratio=0.5)
    ...
    while FE < maxFE:
        ...
        if state.sigma < 1e-5:
            print("Sigma is less than 1e-5, halting the optimization...")
            break
