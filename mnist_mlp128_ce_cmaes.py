...
if __name__ == "__main__":
    ...
    print(out)
    csv_path = f"out/{problem_name}_block_bs{block_size}_gfo_100Kfe_{problem.criterion}_hist.csv"
    plt_path = f"out/{problem_name}_block_bs{block_size}_gfo_100Kfe_{problem.criterion}_plt.pdf"
    model_path = (
        f"out/{problem_name}_block_bs{block_size}_gfo_100Kfe_{problem.criterion}_model"
    )
    ...
    optimizer = CMA_ES(popsize=4 + int(3 * np.log(bD)), num_dims=bD, sigma_init=0.01)
